...
def encode_frame_stack(planes: list[np.ndarray], encoding: str) -> np.ndarray:
    if encoding == "raw":
        return np.stack(planes, axis=0)

    if encoding == "motion":
        anchor = planes[len(planes) // 2]
        diffs = [
            np.abs(curr.astype(np.int16) - prev.astype(np.int16)).astype(np.uint8)
            for prev, curr in zip(planes[:-1], planes[1:])
        ]
        return np.stack([anchor, *diffs], axis=0)

    if encoding == "signed_motion":
        anchor = planes[len(planes) // 2]
        diffs = []
        for prev, curr in zip(planes[:-1], planes[1:]):
            signed = curr.astype(np.int16) - prev.astype(np.int16)
            # Store signed motion in uint8 with 128 as zero-motion so ai8x normalization maps it near 0.
            diffs.append(np.clip(np.round(signed / 2.0 + 128.0), 0, 255).astype(np.uint8))
        return np.stack([anchor, *diffs], axis=0)

    if encoding == "mixed":
        if len(planes) != 12:
            raise ValueError("mixed encoding expects exactly 12 sampled frames")
        raw_indices = [0, 4, 8, 11]
        raw = [planes[index] for index in raw_indices]
        diffs = [
            np.abs(curr.astype(np.int16) - prev.astype(np.int16)).astype(np.uint8)
            for prev, curr in zip(planes[:-1], planes[1:])
        ]
        diff_indices = np.round(np.linspace(0, len(diffs) - 1, 8)).astype(np.int64).tolist()
        return np.stack([*raw, *[diffs[index] for index in diff_indices]], axis=0)

    raise ValueError(f"Unsupported frame encoding: {encoding}")


def read_video(video_dir: Path, frames: int, image_size: int, encoding: str) -> np.ndarray:
    ...
    return encode_frame_stack(planes, encoding)
...
def build_split_cache(
    ...
    for index, (video_id, video_dir, label) in enumerate(samples, start=1):
        try:
            data.append(read_video(video_dir, frames, image_size, encoding))
            labels.append(label)
            ids.append(video_id)
...
def build_cache(args: argparse.Namespace) -> None:
    ...
    build_split_cache("train", train, cache_dir / "train.pt", args.frames, args.image_size, args.encoding)
    build_split_cache("val", val, cache_dir / "val.pt", args.frames, args.image_size, args.encoding)
...
def parse_args() -> argparse.Namespace:
    ...
    cache.add_argument("--encoding", choices=("raw", "motion", "signed_motion", "mixed"), default="raw")
