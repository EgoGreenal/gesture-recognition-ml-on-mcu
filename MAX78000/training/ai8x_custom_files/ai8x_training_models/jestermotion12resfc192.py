###################################################################################################
# 20BN-Jester 12-plane motion-channel residual gesture network with a wider classifier
###################################################################################################
"""Residual motion-channel Jester network with a 192-wide classifier head."""
...
class JesterMotion12ResFC192Net(nn.Module):
    """Motion12 residual CNN with a slightly wider classifier for MAX78000 trials."""
    ...
    def __init__(self, num_classes=27, dimensions=(64, 64), num_channels=12, bias=True, **kwargs):
        ...
        self.fc1 = ai8x.FusedLinearReLU(dim_x * dim_y * 64, 192, bias=True, **kwargs)
        self.fc2 = ai8x.Linear(192, num_classes, wide=True, bias=True, **kwargs)
...
def jestermotion12resfc192(pretrained=False, **kwargs):
    """Construct a residual 12-plane JesterNet with a 192-wide classifier head."""
    assert not pretrained
    return JesterMotion12ResFC192Net(**kwargs)


models = [
    {
        "name": "jestermotion12resfc192",
        "min_input": 1,
        "dim": 2,
    }
]
