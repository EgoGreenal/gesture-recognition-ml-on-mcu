...
BACKUP_ROOT="$PROJECT/runs/ai8x_jestermotion8_${RUN_ID}"
FP_NAME="ai8x_jestermotion8_fp_${RUN_ID}"
QAT_NAME="ai8x_jestermotion8_qat_${RUN_ID}"
...
echo "[$(date)] starting 8-frame motion-channel JesterNet ablation" | tee "$LOG"
echo "project=$PROJECT" | tee -a "$LOG"
echo "cache=$CACHE" | tee -a "$LOG"
echo "encoding=motion: plane0=center frame, plane1-7=abs adjacent differences" | tee -a "$LOG"

if [ ! -f "$CACHE/train.pt" ] || [ ! -f "$CACHE/val.pt" ]; then
  echo "[$(date)] building full motion-channel 8-plane 64x64 cache" | tee -a "$LOG"
...
echo "[$(date)] 8-frame motion-channel JesterNet complete backup_dir=$BACKUP_ROOT" | tee -a "$LOG"
