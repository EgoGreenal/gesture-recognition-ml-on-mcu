...
BACKUP_ROOT="$PROJECT/runs/ai8x_jestermotion12_hard_${RUN_ID}"
FP_NAME="ai8x_jestermotion12_hard_fp_${RUN_ID}"
QAT_NAME="ai8x_jestermotion12_hard_qat_${RUN_ID}"
...
echo "[$(date)] starting hard-class motion12 JesterNet run" | tee "$LOG"
echo "project=$PROJECT" | tee -a "$LOG"
echo "cache=$CACHE" | tee -a "$LOG"
echo "hard classes: sliding two fingers L/R x2; pushing/pulling two fingers, rolling, turning x3" | tee -a "$LOG"
...
echo "[$(date)] floating-point pretrain with hard-class oversampling" | tee -a "$LOG"
python train.py \
  --lr 0.0005 \
  --optimizer adam \
  --epochs "$FP_EPOCHS" \
...
echo "[$(date)] QAT fine-tune with hard-class oversampling" | tee -a "$LOG"
python train.py \
  --lr 0.00005 \
  --optimizer adam \
  --epochs "$QAT_EPOCHS" \
  ...
  --qat-policy policies/qat_policy_jestermotion12_hard.yaml \
  --model jestermotion12 \
  --dataset jester_motion12_hard \
  --data "$CACHE" \
  --validation-split 0 \
...
echo "[$(date)] hard-class motion12 JesterNet complete backup_dir=$BACKUP_ROOT" | tee -a "$LOG"
