...
DISTILL_DIR="$PROJECT/runs/jester_huge_teacher_distill_${RUN_ID}"
BACKUP_ROOT="$PROJECT/runs/ai8x_jestermotion12resxconv_signed_huge_teacher_distill_${RUN_ID}"
LOG="$OUT_DIR/jester_huge_teacher_distill_${RUN_ID}.log"
QAT_NAME="ai8x_jestermotion12resxconv_signed_huge_teacher_distill_qat_${RUN_ID}"

TEACHER_VARIANT="${TEACHER_VARIANT:-huge}"
TEACHER_EPOCHS="${TEACHER_EPOCHS:-90}"
...
echo "[$(date)] Jester huge-teacher distillation started" | tee "$LOG"
echo "project=$PROJECT" | tee -a "$LOG"
echo "cache=$CACHE" | tee -a "$LOG"
echo "teacher_variant=$TEACHER_VARIANT" | tee -a "$LOG"
echo "teacher_epochs=$TEACHER_EPOCHS student_epochs=$STUDENT_EPOCHS qat_epochs=$QAT_EPOCHS batch_size=$BATCH_SIZE" | tee -a "$LOG"
...
echo "[$(date)] Jester huge-teacher distillation finished backup_dir=$BACKUP_ROOT" | tee -a "$LOG"
