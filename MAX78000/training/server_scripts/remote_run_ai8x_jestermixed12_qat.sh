...
CACHE="$PROJECT/cache/jester_cache_mixed12_64_full"
ENV_DIR="$PROJECT/envs/ai8x-py311"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$PROJECT/runtime/ai8x_runs"
SCRATCH_TMP="$PROJECT/tmp"
BACKUP_ROOT="$PROJECT/runs/ai8x_jestermixed12_${RUN_ID}"
FP_NAME="ai8x_jestermixed12_fp_${RUN_ID}"
QAT_NAME="ai8x_jestermixed12_qat_${RUN_ID}"
...
echo "[$(date)] starting mixed raw/motion 12-plane JesterNet run" | tee "$LOG"
echo "project=$PROJECT" | tee -a "$LOG"
echo "cache=$CACHE" | tee -a "$LOG"
echo "encoding=mixed: 4 raw frames at sampled indices 0/4/8/11 + 8 evenly sampled abs adjacent differences" | tee -a "$LOG"

if [ ! -f "$CACHE/train.pt" ] || [ ! -f "$CACHE/val.pt" ]; then
  echo "[$(date)] building full mixed 12-plane 64x64 cache" | tee -a "$LOG"
...
python train.py \
  ...
  --qat-policy policies/qat_policy_jestermixed12.yaml \
  --model jestermixed12 \
  --dataset jester_mixed12 \
  --data "$CACHE" \
  --validation-split 0 \
...
echo "[$(date)] mixed raw/motion JesterNet complete backup_dir=$BACKUP_ROOT" | tee -a "$LOG"
