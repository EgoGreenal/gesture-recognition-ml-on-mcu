...
#define ENABLE_EARLY_EXIT_POLICY 1
#define USE_SIGNED_MOTION_INPUT 0
#define ENABLE_STAGE_PROFILE 1
#define PROFILE_TIMER MXC_TMR2
#define MODEL_NAME "jestermotion12_qat_earlystop"
#define EARLY_EXIT_FIRST_PREFIX 8
#define EARLY_EXIT_SECOND_PREFIX 10
#define EARLY_EXIT_FULL_PREFIX 12
#define EARLY_STOP_PREFIX8_SKIP_FRAMES 4
#define EARLY_STOP_PREFIX10_SKIP_FRAMES 2
...
static int early_stop_skip_frames_for_prefix(int prefix)
{
    if (prefix == EARLY_EXIT_FIRST_PREFIX) {
        return EARLY_STOP_PREFIX8_SKIP_FRAMES;
    }

    if (prefix == EARLY_EXIT_SECOND_PREFIX) {
        return EARLY_STOP_PREFIX10_SKIP_FRAMES;
    }
...
static void select_best_output(int *best_class, q15_t *best_score, int32_t *best_logit)
{
    *best_score = -32768;
...
static void run_cnn_probe(int prefix_planes, profile_times_t *profile)
{
#if ENABLE_STAGE_PROFILE
    MXC_TMR_SW_Start(PROFILE_TIMER);
#endif
    load_input(prefix_planes);
#if ENABLE_STAGE_PROFILE
...
void process_img(uint32_t frame_index, int send_image, uint32_t capture_wait_us)
{
    ...
    if (policy_cooldown_frames > 0) {
        ...
        skipped = 1;
        cnn_us = 0;
        prefix = policy_hold_prefix;
        policy = policy_hold_prefix == EARLY_EXIT_FIRST_PREFIX ? "hold8" : "hold10";
        best_class = last_best_class;
        best_score = last_best_score;
        best_logit = last_best_logit;
        if (policy_cooldown_frames == 0) {
            policy_hold_prefix = 0;
        }
        goto send_packet;
    }
    ...
    policy = "prefix8";
    run_cnn_probe(prefix, &profile);
    select_best_output(&best_class, &best_score, &best_logit);
    ...
    if (prefix == EARLY_EXIT_FIRST_PREFIX || prefix == EARLY_EXIT_SECOND_PREFIX) {
        if (accepted) {
            policy_cooldown_frames = early_stop_skip_frames_for_prefix(prefix);
            policy_hold_prefix = prefix;
        }
    } else {
        policy_hold_prefix = 0;
    }
