...
在 STM32U5 (B-U585I-IOT02A, Cortex-M33 @ 160 MHz, 786 KB SRAM, 2 MB Flash) 上部署了一个实时手势识别系统：27-class Jester @ INT8 PTQ accuracy 86.49%，延迟 199 ms/clip（早退 S1 mf=5 thresh=0.85，平均 obs 0.776），Flash 0.99 MB / SRAM 261 KB。教师 → TA → student 三阶段 KD 蒸馏 (TAKD) + 17 层 TSM-MBV2 cone 截断 (w=0.5) + per-channel symmetric INT8 PTQ + min_exit_frame 约束早退。所有 U5 硬约束均通过（Flash<2MB / SRAM<600KB budget / lat<150ms/frame）。
...
## 10. 结论与未来工作
- 实现了 STM32U5 上 27-class 实时手势识别 @ INT8 PTQ 86.69% / lat 199 ms / Flash 0.99 MB
- 早退策略 mf=5 + S1=0.85 节省 22% 推理时间 + 不降精度
- 未来方向：
